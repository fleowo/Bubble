...
ElementsTable.Toggle = (function()
	...
	function Element:New(Idx, Config)
		...
		function Toggle:AddKeybind(Idx, Config)
		...
		local KeybindDisplayFrame = New("TextButton", {
			...
			function KeybindToggle:GetState()
				if UserInputService:GetFocusedTextBox() and KeybindToggle.Mode ~= "Always" then
					return false
				end
	
				if KeybindToggle.Mode == "Always" then
					return true
				elseif KeybindToggle.Mode == "Hold" then
					if KeybindToggle.Value == "None" then
						return false
					end
	
					local Key = KeybindToggle.Value
					...
					if Key == "MouseLeft" or Key == "MouseRight" then
						...
						return UserInputService:IsKeyDown(Enum.KeyCode[KeybindToggle.Value])
					end
				else
					return KeybindToggle.Toggled
				end
			end
	
			function KeybindToggle:SetValue(Key, Mode)
				Key = Key or KeybindToggle.Key
				Mode = Mode or KeybindToggle.Mode
	
				KeybindDisplayLabel.Text = Key
				KeybindToggle.Value = Key
				KeybindToggle.Mode = Mode
			end
	
			function KeybindToggle:OnClick(Callback)
				KeybindToggle.Clicked = Callback
			end
	
			function KeybindToggle:OnChanged(Callback)
				KeybindToggle.Changed = Callback
				Callback(KeybindToggle.Value)
			end
	
			function KeybindToggle:DoClick()
				Library:SafeCallback(KeybindToggle.Callback, KeybindToggle.Toggled)
				Library:SafeCallback(KeybindToggle.Clicked, KeybindToggle.Toggled)
			end
	
			function KeybindToggle:Destroy()
				Toggle.Elements.Frame:Destroy()
				Library.Options[Idx] = nil
			end
					...
					Event = UserInputService.InputBegan:Connect(function(Input)
						...
						EndedEvent = UserInputService.InputEnded:Connect(function(Input)
							...
							then
								...
								Library:SafeCallback(KeybindToggle.ChangedCallback, Input.KeyCode or Input.UserInputType)
								Library:SafeCallback(KeybindToggle.Changed, Input.KeyCode or Input.UserInputType)
			...
			Creator.AddSignal(UserInputService.InputBegan, function(Input)
				if not Picking and not UserInputService:GetFocusedTextBox() then
					if KeybindToggle.Mode == "Toggle" then
						local Key = KeybindToggle.Value
						...
						if Key == "MouseLeft" or Key == "MouseRight" then
							if
								...
								or Key == "MouseRight" and Input.UserInputType == Enum.UserInputType.MouseButton2
							then
								KeybindToggle.Toggled = not KeybindToggle.Toggled
								KeybindToggle:DoClick()
							end
			...
			Library.Options[Idx] = KeybindToggle
		...
		function Toggle:AddColorPicker(Idx, Config)
			...
			function ColorpickerToggle:SetHSVFromRGB(Color)
				local H, S, V = Color3.toHSV(Color)
				ColorpickerToggle.Hue = H
				ColorpickerToggle.Sat = S
				ColorpickerToggle.Vib = V
			end
	
			ColorpickerToggle:SetHSVFromRGB(ColorpickerToggle.Value)
	
			local DisplayFrameColor = New("Frame", {
				Size = UDim2.fromScale(1, 1),
				BackgroundColor3 = ColorpickerToggle.Value,
				Parent = Toggle.Elements.Frame,
			}, {
				New("UICorner", {
					CornerRadius = UDim.new(0, 4),
			...
			local DisplayFrame = New("ImageButton", {
			...
			local function CreateColorDialog()
				...
				Dialog.Title.Text = ColorpickerToggle.Title
				Dialog.Root.Size = UDim2.fromOffset(430, 330)
	
				local Hue, Sat, Vib = ColorpickerToggle.Hue, ColorpickerToggle.Sat, ColorpickerToggle.Vib
				local Transparency = ColorpickerToggle.Transparency
				...
				local OldColorFrame = New("Frame", {
					BackgroundColor3 = ColorpickerToggle.Value,
					Size = UDim2.fromScale(1, 1),
					BackgroundTransparency = ColorpickerToggle.Transparency,
				}, {
					New("UICorner", {
						CornerRadius = UDim.new(0, 4),
					}),
				...
				Dialog:Button("Done", function()
					ColorpickerToggle:SetValue({ Hue, Sat, Vib }, Transparency)
				end)
				Dialog:Button("Cancel")
				Dialog:Open()
			end
	
			function ColorpickerToggle:Display()
				ColorpickerToggle.Value = Color3.fromHSV(ColorpickerToggle.Hue, ColorpickerToggle.Sat, ColorpickerToggle.Vib)
	
				DisplayFrameColor.BackgroundColor3 = ColorpickerToggle.Value
				DisplayFrameColor.BackgroundTransparency = ColorpickerToggle.Transparency
	
				Element.Library:SafeCallback(ColorpickerToggle.Callback, ColorpickerToggle.Value)
				Element.Library:SafeCallback(ColorpickerToggle.Changed, ColorpickerToggle.Value)
			end
	
			function ColorpickerToggle:SetValue(HSV, Transparency)
				local Color = Color3.fromHSV(HSV[1], HSV[2], HSV[3])
	
				ColorpickerToggle.Transparency = Transparency or 0
				ColorpickerToggle:SetHSVFromRGB(Color)
				ColorpickerToggle:Display()
			end
	
			function ColorpickerToggle:SetValueRGB(Color, Transparency)
				ColorpickerToggle.Transparency = Transparency or 0
				ColorpickerToggle:SetHSVFromRGB(Color)
				ColorpickerToggle:Display()
			end
	
			function ColorpickerToggle:OnChanged(Func)
				ColorpickerToggle.Changed = Func
				Func(ColorpickerToggle.Value)
			end
			
			function ColorpickerToggle:Destroy()
				DisplayFrame:Destroy()
				Library.Options[Idx] = nil
			end
	
			Creator.AddSignal(DisplayFrame.MouseButton1Click, function()
				CreateColorDialog()
			end)
	
			ColorpickerToggle:Display()
	
			Library.Options[Idx] = ColorpickerToggle
	...
	SaveManager.Parser = {
		Toggle = {
			Save = function(idx, object) 
				...
				if object.KeybindToggle then
					data.KeybindToggle = {
						key = object.KeybindToggle.Value,
						mode = object.KeybindToggle.Mode
					}
				end

                if object.ColorpickerToggle then
                    data.ColorpickerToggle = {
			...
			Load = function(idx, data)
				if SaveManager.Options[idx] then 
					...
					if data.KeybindToggle and SaveManager.Options[idx].KeybindToggle then
						SaveManager.Options[idx].KeybindToggle:SetValue(data.KeybindToggle.key, data.KeybindToggle.mode)
					end

                    if data.ColorpickerToggle and SaveManager.Options[idx].ColorpickerToggle then
						SaveManager.Options[idx].ColorpickerToggle:SetValueRGB(Color3.fromHex(data.ColorpickerToggle.value), data.ColorpickerToggle.transparency)
					end
				end
			end,
		},
...
function Library:CreateWindow(Config)
	...
	local MinimizeButton = New("TextButton", {
		...
		New("ImageLabel", {
			Image = Config.MinimizerIcon or "rbxassetid://90914900554954" ,
			Size = UDim2.new(1, 0, 1, 0),
			BackgroundTransparency = 1,
		}, {
